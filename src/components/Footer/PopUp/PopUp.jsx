  ...
  return createPortal(
    <Overlay onClick={handleBackdropClick}>
      <Modal $modal={`${isModalOpen}`}>
        ...
        <p data={currentLanguage} className="text">{t('modal-text')}</p>
        <ul className="list">
          {data.map(({ id, image, teamRole,teamRoleUA, teamNameUA, teamName, projectUrl }) => (
            <li className="li-item" key={id}>
              <a className="link" href={projectUrl} target="_blank" rel="noreferrer noopener">
                ...
                <p className="team-role">{currentLanguage === 'en' ? teamRole : teamRoleUA}</p>
                <p className="team-name">{currentLanguage === 'en' ? teamName : teamNameUA}</p>
              </a>
            </li>
          ))}
        </ul>
